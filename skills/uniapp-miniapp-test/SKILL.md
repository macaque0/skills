...
- 页面是否有明显错位、遮挡、溢出、重叠、文字截断或图片拉伸。
- 顶部导航栏、底部 TabBar、固定按钮、弹窗、Toast、Loading、空状态是否位置正确。
- iPhone 刘海屏、底部安全区、不同屏宽下的布局是否可用。
- 长标题、长昵称、长金额、长地址、多行说明等极端文本是否仍能展示。
- 深色模式、主题色、按钮禁用态、选中态、错误态是否与页面设计一致；如项目不支持深色模式，应确认没有异常反色。
- 列表滚动、分页加载、下拉刷新、弹窗滚动、键盘弹起时是否遮挡输入框或提交按钮。
- 截图对比时，记录设备尺寸、页面路径、关键状态和截图路径；如果只能人工查看，输出明确检查点。
